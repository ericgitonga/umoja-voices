...
- **Navbar logo**: both the signed-in `Nav` and the logged-out `PublicNav` now show
  ...
  read at navbar height) instead of a plain `♪` character. Generated by cropping the icon region
  from the app owner's supplied logo image, then removing the white background with alpha
  decontamination (not just naive white-to-transparent, which left a visible light halo around
  the anti-aliased edges from JPEG compression) so it reads cleanly on both the dark navbar and
  any future light-background use.
